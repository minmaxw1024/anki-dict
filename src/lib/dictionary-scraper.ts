...
function extractPronunciations(doc: Document): WordEntry["pronunciations"] {
  ...
  if (ukPronContainer) {
    ...
    const audioElement = ukPronContainer.querySelector(
  ...
  if (usPronContainer) {
    ...
    const audioElement = usPronContainer.querySelector(
      'source[type="audio/mpeg"]',
    );
...
function extractDefinitions(container: Element): Definition[] {
  ...
  const posElement = findElement(
    container,
    CAMBRIDGE_SELECTORS.selectors.partOfSpeech,
  );
  const partOfSpeech = posElement?.textContent?.trim() || "unknown";

  const defBlocks = findElements(
    container,
    CAMBRIDGE_SELECTORS.selectors.definitionBlocks,
  );

  if (defBlocks.length === 0) {
    const meaningElement = findElement(
      container,
      CAMBRIDGE_SELECTORS.selectors.meaning,
    );
    if (meaningElement) {
      const examples = findElements(
        container,
        CAMBRIDGE_SELECTORS.selectors.examples,
      )
    ...
    for (const block of defBlocks) {
      ...
      const levelElement = findElement(
        block,
        CAMBRIDGE_SELECTORS.selectors.level,
      );
...
export function parseCambridgeHtml(html: string, word: string): WordEntry {
  const cleanWord = word.toLowerCase().trim();
